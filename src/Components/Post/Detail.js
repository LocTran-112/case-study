...
const Detail = (props) => {
  ...
  const { user, setIsLoading, selectedPost, setSelectedPost } = useContext(Context);
  ...
  loadPost = useCallback(async () => {
    ...
    try {
      ...
      const url = `https://6499a33d79fbe9bcf83faadd.mockapi.io/post/${id}`;
  ...
  const toggleFollow = async () => {
    try {
      if (post.hasFollowed) {
        ...
        const customMessage = { message: `Someone has liked your post`, type: 'notification'};
        ...
        <div className="post-detail__main">
          ...
          <div className="post-detail__main-right">
            <div className="post-detail__creator">
              <img src={`https://6499a33d79fbe9bcf83faadd.mockapi.io/user_info${post?.avatar}`} alt={user.avatar} />
